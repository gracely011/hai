# Gracely Web & Extension Rules

- **DO NOT FETCH PRODUCTION JS:** JANGAN PERNAH mengambil (fetch) atau menganalisis file berformat `.js` langsung dari situs web aslinya (misalnya `draft.gracely.my.id`). File JavaScript di server production sudah dienkripsi (obfuscated) dan tidak akan bisa diurai dengan benar. Selalu gunakan versi `.js` lokal yang ada di dalam workspace (`d:\DOCUMENT\WEB\hai-main antigravity` atau `d:\OTHER\gracely-extension-2.2`).
- **HUBUNGAN FOLDER (PROJECT LINKING):** Folder `d:\OTHER\gracely-extension-2.2` dan `d:\OTHER\gracely-guard-1.0.3` sangat terikat (linked) dengan folder `d:\DOCUMENT\WEB\hai-main antigravity`. Kedua ekstensi tersebut memuat/menggunakan konfigurasi utama dari `aturhonma.js` yang berada di folder `hai-main antigravity`. Jika ada masalah pada ekstensi, selalu periksa konfigurasi di `hai-main antigravity\aturhonma.js`.
- **ZIP EXTENSION:** JANGAN PERNAH membuat arsip (zip) dari folder `gracely-extension-2.2` dan `gracely-guard-1.0.3` secara otomatis. Pengguna memiliki mekanisme/alur tersendiri untuk mengelola dan membuat file zip tersebut. Cukup langsung lakukan commit dan push pada file zip yang sudah ada di dalam direktori `assets/update/`.
